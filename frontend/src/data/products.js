// Product catalog data. Add new products by appending objects here.
// - Products WITH an `image` render as visual cards in the catalog.
// - Products with `image` left as empty string render as a text-only list row.
// - Prices are intentionally NOT shown anywhere on the site (enquire by phone/email).
//
// HOW TO ADD A PRODUCT:
//   1. Copy any object below.
//   2. Give it a NEW unique `id` and `slug`.
//   3. Fill in name, category, specs and description.
//   4. Set `image` to a URL to render as a card, or "" to render as a list row.

export const CATEGORIES = [
    "Molecular Diagnostics",
    "Mindray Hematology",
    "Polymed",
    "Other",
];

export const PRODUCTS = [
    {
        id: 1,
        slug: "truelab-uno-dx",
        name: "Truelab Uno Dx",
        category: "Molecular Diagnostics",
        tagline:
            "Portable single-bay RT-PCR analyser — compact molecular diagnostics for low-volume testing.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/p9velohh_truelab_Uno_Dx.webp",
        code: "SB-MD001",
        specs: [
            { label: "Brand", value: "Molbio" },
            { label: "Model", value: "Truelab Uno Dx" },
            { label: "Throughput", value: "10 – 12 samples per 8 hours" },
            { label: "Testing Bays", value: "1 (Single)" },
            { label: "Category", value: "Molecular Diagnostics" },
        ],
        features: [
            "Single testing bay for streamlined diagnostics",
            "Compact footprint ideal for low-volume testing",
            "Point-of-care ready — battery-operable field deployment",
        ],
        description:
            "The Truelab Uno Dx is a compact, single-bay real-time quantitative micro-PCR analyser designed for point-of-care molecular diagnostics. Its streamlined workflow and small footprint make it ideal for clinics, remote sites and low-volume testing environments where portability and simplicity matter.",
    },
    {
        id: 2,
        slug: "truelab-duo",
        name: "Truelab Duo",
        category: "Molecular Diagnostics",
        tagline:
            "Dual-bay RT-PCR analyser with complete random access — run two assays simultaneously.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/hf1ke6dt_Truelab_duo.webp",
        code: "SB-MD002",
        specs: [
            { label: "Brand", value: "Molbio" },
            { label: "Model", value: "Truelab Duo" },
            { label: "Throughput", value: "20 – 24 samples per 8 hours" },
            { label: "Testing Bays", value: "2 (Dual, random access)" },
            { label: "Category", value: "Molecular Diagnostics" },
        ],
        features: [
            "Dual testing bays for enhanced flexibility",
            "Complete random access — run different assays in parallel",
            "Twice the throughput without doubling the footprint",
        ],
        description:
            "The Truelab Duo is a dual-bay real-time micro-PCR analyser with complete random access, letting labs run two independent assays simultaneously. Ideal for mid-volume clinical labs and diagnostic centres that need flexibility and parallel testing without a full-scale PCR setup.",
    },
    {
        id: 3,
        slug: "truelab-quattro",
        name: "Truelab Quattro",
        category: "Molecular Diagnostics",
        tagline:
            "Four-bay high-throughput RT-PCR platform for high-volume molecular diagnostics.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/xmoczz1m_Quattro.webp",
        code: "SB-MD003",
        specs: [
            { label: "Brand", value: "Molbio" },
            { label: "Model", value: "Truelab Quattro" },
            { label: "Throughput", value: "40 – 48 samples per 8 hours" },
            { label: "Testing Bays", value: "4 (Quad, random access)" },
            { label: "Category", value: "Molecular Diagnostics" },
        ],
        features: [
            "Four independent testing bays for high-volume diagnostics",
            "Complete random access for an optimised, parallel workflow",
            "Scales molecular testing without expanding the physical bench",
        ],
        description:
            "The Truelab Quattro is a high-throughput four-bay real-time micro-PCR system that scales molecular testing for busy diagnostic labs, hospital reference centres and public-health screening programmes. Complete random access across all four bays keeps the workflow optimised even when multiple assays run in parallel.",
    },
    {
        id: 4,
        slug: "mindray-bc-20-auto-hematology-analyzer",
        name: "Mindray BC-20 Auto Hematology Analyzer",
        category: "Mindray Hematology",
        tagline: "3-part hematology analyzer — Mindray.",
        image: "",
        code: "SB-MH004",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BC-20 Auto Hematology Analyzer" },
            { label: "Throughput", value: "60 samples per hour" },
            { label: "Parameters", value: "19 parameters + 3 histograms" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Compact 3-part differential CBC analyzer",
            "Colour touchscreen with intuitive workflow",
            "Closed-tube sampling & cap-piercing options",
        ],
        description:
            "The Mindray BC-20 is an entry-level 3-part differential auto hematology analyzer. Compact and dependable, it delivers CBC results across 19 parameters and three histograms — ideal for small labs, clinics and outpatient centres.",
    },
    {
        id: 5,
        slug: "mindray-bc-700-hematology-esr-analyzer",
        name: "Mindray BC-700 Hematology & ESR Analyzer",
        category: "Mindray Hematology",
        tagline: "7-part hematology + ESR — Mindray.",
        image: "",
        code: "SB-MH005",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BC-700 Hematology & ESR Analyzer" },
            { label: "Throughput", value: "Up to 60 samples per hour" },
            { label: "Parameters", value: "37 parameters + integrated ESR" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "First in class to combine 5-part CBC with integrated ESR",
            "Closed-tube reagent management",
            "Onboard QC with Levey-Jennings tracking",
        ],
        description:
            "The Mindray BC-700 series is the industry's first analyzer to combine 5-part CBC with integrated ESR in a single walkaway workflow. Ideal for mid-to-high volume clinical labs seeking to consolidate haematology and ESR onto one platform.",
    },
    {
        id: 6,
        slug: "mindray-bc-720-auto-hematology-analyzer",
        name: "Mindray BC-720 Auto Hematology Analyzer",
        category: "Mindray Hematology",
        tagline: "5-part hematology analyzer — Mindray.",
        image: "",
        code: "SB-MH006",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BC-720 Auto Hematology Analyzer" },
            { label: "Throughput", value: "60 samples per hour" },
            { label: "Parameters", value: "29 parameters + 3 histograms + 2 scattergrams" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "SF Cube 3D cell-analysis technology",
            "Auto-loader with continuous tube handling",
            "Compact benchtop footprint",
        ],
        description:
            "The Mindray BC-720 is a compact 5-part differential auto hematology analyzer using SF Cube 3D cell-analysis technology for reliable classification. Designed for medium-volume clinical labs that need throughput without the space overhead.",
    },
    {
        id: 7,
        slug: "mindray-bc-760-auto-hematology-analyzer",
        name: "Mindray BC-760 Auto Hematology Analyzer",
        category: "Mindray Hematology",
        tagline: "5-part hematology with reticulocyte — Mindray.",
        image: "",
        code: "SB-MH007",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BC-760 Auto Hematology Analyzer" },
            { label: "Throughput", value: "90 samples per hour" },
            { label: "Parameters", value: "35+ parameters + reticulocyte channel" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Dedicated reticulocyte channel for anaemia workup",
            "SF Cube 3D cell classification",
            "Automated CBC + DIFF + RET on a single platform",
        ],
        description:
            "The Mindray BC-760 auto hematology analyzer delivers 5-part CBC/DIFF and a dedicated reticulocyte channel using Mindray's SF Cube 3D cell-classification technology. A strong fit for hospital labs where anaemia workup volume is high.",
    },
    {
        id: 8,
        slug: "mindray-bc-780-auto-hematology-analyzer",
        name: "Mindray BC-780 Auto Hematology Analyzer",
        category: "Mindray Hematology",
        tagline: "6-part hematology with reticulocyte — Mindray.",
        image: "",
        code: "SB-MH008",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BC-780 Auto Hematology Analyzer" },
            { label: "Throughput", value: "120 samples per hour" },
            { label: "Parameters", value: "40+ parameters, 6-part diff + reticulocyte + NRBC" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "6-part differential including immature granulocytes",
            "Reticulocyte and NRBC counting on-board",
            "Auto-loader with STAT sample interrupt",
        ],
        description:
            "The Mindray BC-780 is a high-throughput 6-part differential auto hematology analyzer with on-board reticulocyte and NRBC counting. Suited to large hospital and reference laboratories running heavy haematology volumes.",
    },
    {
        id: 9,
        slug: "mindray-bs-230-fully-auto-biochemistry-analyzer",
        name: "Mindray BS-230 Fully-Auto Biochemistry Analyzer",
        category: "Mindray Hematology",
        tagline: "Biochemistry analyzer — Mindray.",
        image: "",
        code: "SB-MH009",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BS-230 Fully-Auto Biochemistry Analyzer" },
            { label: "Throughput", value: "200 tests per hour (up to 240 with ISE)" },
            { label: "Parameters", value: "Photometric + optional ISE (Na/K/Cl)" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Random access with STAT priority",
            "Reagent barcode recognition",
            "Long-life halogen lamp with 12 wavelengths",
        ],
        description:
            "The Mindray BS-230 is a fully automated random-access biochemistry analyzer delivering 200 photometric tests per hour, expandable to 240 tests/hour with the optional ISE module. Reliable throughput for small and mid-sized clinical laboratories.",
    },
    {
        id: 10,
        slug: "mindray-bs-240-fully-auto-biochemistry-analyzer",
        name: "Mindray BS-240 Fully-Auto Biochemistry Analyzer",
        category: "Mindray Hematology",
        tagline: "Biochemistry analyzer — Mindray.",
        image: "",
        code: "SB-MH010",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BS-240 Fully-Auto Biochemistry Analyzer" },
            { label: "Throughput", value: "240 tests per hour (up to 400 with ISE)" },
            { label: "Parameters", value: "Photometric + optional ISE (Na/K/Cl)" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Constant-temperature reagent tray",
            "Automatic sample dilution & rerun",
            "Bi-directional LIS interfacing",
        ],
        description:
            "The Mindray BS-240 is a compact fully-auto biochemistry analyzer running 240 tests per hour, expandable to 400 tests/hour with the ISE module. A dependable workhorse for busy mid-volume clinical chemistry labs.",
    },
    {
        id: 11,
        slug: "mindray-bs-240e-fully-auto-biochemistry-analyzer",
        name: "Mindray BS-240E Fully-Auto Biochemistry Analyzer",
        category: "Mindray Hematology",
        tagline: "Biochemistry analyzer, economy — Mindray.",
        image: "",
        code: "SB-MH011",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BS-240E Fully-Auto Biochemistry Analyzer" },
            { label: "Throughput", value: "240 tests per hour" },
            { label: "Parameters", value: "Photometric, 12 wavelengths" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Value-focused variant of the BS-240 platform",
            "Continuous loading of 40 samples + 40 reagents",
            "Built-in QC with real-time flagging",
        ],
        description:
            "The Mindray BS-240E is an economy variant of the BS-240 platform, delivering 240 tests per hour without compromising on reliability. Ideal for growing clinical labs that want the BS-240 workflow at an accessible investment.",
    },
    {
        id: 12,
        slug: "mindray-bs-430-fully-auto-biochemistry-analyzer",
        name: "Mindray BS-430 Fully-Auto Biochemistry Analyzer",
        category: "Mindray Hematology",
        tagline: "High-throughput biochemistry — Mindray.",
        image: "",
        code: "SB-MH012",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BS-430 Fully-Auto Biochemistry Analyzer" },
            { label: "Throughput", value: "400 tests per hour (up to 600 with ISE)" },
            { label: "Parameters", value: "Photometric + integrated ISE" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "High-throughput 400+ tests/hour",
            "Integrated ISE for Na/K/Cl",
            "Auto sample dilution, rerun and reflex testing",
        ],
        description:
            "The Mindray BS-430 is a high-throughput fully-auto biochemistry analyzer delivering 400 tests per hour, extending to 600 tests/hour with integrated ISE. Built for medium-to-large hospital and reference laboratories.",
    },
    {
        id: 13,
        slug: "mindray-ba-88a-semi-auto-biochemistry-analyzer",
        name: "Mindray BA-88A Semi-Auto Biochemistry Analyzer",
        category: "Mindray Hematology",
        tagline: "Semi-auto biochemistry analyzer — Mindray.",
        image: "",
        code: "SB-MH013",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "BA-88A Semi-Auto Biochemistry Analyzer" },
            { label: "Throughput", value: "Manual sampling, batch throughput" },
            { label: "Parameters", value: "Photometric, 8 filter wavelengths" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Compact benchtop semi-auto analyzer",
            "Peltier-cooled flow cell for stable readings",
            "Thermal printer and full-colour touchscreen",
        ],
        description:
            "The Mindray BA-88A is a dependable semi-automatic biochemistry analyzer with a Peltier-cooled flow cell and 8-wavelength photometry. A practical entry-point for smaller labs, satellite clinics and back-up biochemistry lines.",
    },
    {
        id: 14,
        slug: "mindray-cl-900i-chemiluminescence-immunoassay-analyzer",
        name: "Mindray CL-900i Chemiluminescence Immunoassay Analyzer",
        category: "Mindray Hematology",
        tagline: "Chemiluminescence immunoassay — Mindray.",
        image: "",
        code: "SB-MH014",
        specs: [
            { label: "Brand", value: "Mindray" },
            { label: "Model", value: "CL-900i Chemiluminescence Immunoassay Analyzer" },
            { label: "Throughput", value: "Up to 180 tests per hour" },
            { label: "Parameters", value: "Full CLIA menu — thyroid, cardiac, fertility, tumour, infectious disease" },
            { label: "Category", value: "Mindray Hematology" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Fully automated chemiluminescence workflow",
            "Continuous reagent loading with onboard cooling",
            "Broad menu covering thyroid, cardiac, tumour, fertility, infectious disease",
        ],
        description:
            "The Mindray CL-900i is a fully automated chemiluminescence immunoassay analyzer running up to 180 tests per hour. Its broad assay menu — thyroid, cardiac, fertility, tumour markers, infectious disease — makes it a strong immunoassay backbone for growing clinical labs.",
    },
    {
        id: 15,
        slug: "multi-sample-blood-collection-needles",
        name: "Multi-Sample Blood Collection Needles",
        category: "Polymed",
        tagline: "Blood Collection Needles — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/j9xh4vx1_Blood-Collection-Needles-93-507x1536.webp",
        code: "SB-PM009",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "Blood Collection Needles" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Essential for venipuncture and blood sample collection",
            "Designed for comfort and efficiency during draw",
            "Compatible with various blood collection tubes and systems",
            "Ensures safe and precise blood draw",
            "Available in various sizes for different needs",
        ],
        description:
            "Polymed multi-sample blood collection needles are essential for venipuncture and blood sample collection. Designed for comfort and efficiency, they are compatible with all standard blood collection tubes and holders, ensuring safe and precise blood draw across a range of gauges suited to different patient needs.",
    },
    {
        id: 16,
        slug: "haemoflash",
        name: "Haemoflash",
        category: "Polymed",
        tagline: "Flashback needle — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/rdotqg6h_Haemoflash-2-1-300x300-1.webp",
        code: "SB-PM010",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "Flashback needle" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Instant flashback to confirm successful needle placement",
            "Reduces failed sticks and improves first-attempt success",
            "Ergonomic hub for a stable, low-fatigue grip",
        ],
        description:
            "Polymed Haemoflash provides instant flashback to confirm successful needle placement, reducing failed sticks and improving the first-attempt success rate for phlebotomists.",
    },
    {
        id: 17,
        slug: "esr-erythrocyte-sedimentation-rate-tubes",
        name: "ESR (Erythrocyte Sedimentation Rate) Tubes",
        category: "Polymed",
        tagline: "ESR tubes — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/7qshotkd_ESR-Erythrocyte-Sedimentation-Rate-Tubes-90-600x753.webp",
        code: "SB-PM011",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "ESR tubes" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Contains 3.8% Buffered Sodium Citrate",
            "Ideal for Westergren sedimentation rate determination",
            "Consistent draw volume for reproducible readings",
        ],
        description:
            "Polymed ESR tubes contain 3.8% buffered sodium citrate and are used for Westergren sedimentation rate determination. Consistent draw volumes give reproducible readings across routine haematology screening.",
    },
    {
        id: 18,
        slug: "lithium-heparin-tubes",
        name: "Lithium Heparin Tubes",
        category: "Polymed",
        tagline: "Lithium Heparin tubes — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/6dhlj6vb_Lithium-Heparin-Tubes-88-600x691.webp",
        code: "SB-PM012",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "Lithium Heparin tubes" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Coated with Sodium or Lithium Heparin as anticoagulant",
            "Anticoagulant activates antithrombins, blocking the coagulation cascade",
            "Produces whole blood — suitable for plasma determinations in chemistry",
        ],
        description:
            "Polymed Lithium Heparin tubes are coated with sodium or lithium heparin, an anticoagulant that activates antithrombins to block the coagulation cascade. Produces whole blood suitable for plasma determinations in chemistry assays.",
    },
    {
        id: 19,
        slug: "sodium-heparin-tubes",
        name: "Sodium Heparin Tubes",
        category: "Polymed",
        tagline: "Sodium Heparin tubes — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/x6iqsj02_Sodium-Heparin-Tubes-87-600x743.webp",
        code: "SB-PM013",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "Sodium Heparin tubes" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Prevents clotting — ensures accurate plasma analysis",
            "Used for various clinical tests including chemistry and haematology assays",
            "Consistent additive coating for reproducible results",
        ],
        description:
            "Polymed Sodium Heparin tubes prevent clotting and ensure accurate plasma analysis. Widely used for chemistry and haematology assays where a whole-blood or plasma matrix is required.",
    },
    {
        id: 20,
        slug: "glucose-estimation-tubes",
        name: "Glucose Estimation Tubes",
        category: "Polymed",
        tagline: "Glucose estimation tubes — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/1x27k0xs_Glucose-Estimation-Tubes-86-600x733.webp",
        code: "SB-PM014",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "Glucose estimation tubes" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Contains anticoagulant and glycolytic inhibitor",
            "Suitable for blood sugar determinations",
            "Also suitable for lactate determinations",
        ],
        description:
            "Polymed Glucose Estimation tubes contain anticoagulant plus a glycolytic inhibitor to stabilise glucose in the drawn sample. Suitable for blood sugar and lactate determinations.",
    },
    {
        id: 21,
        slug: "hematology-edta-k3-tube",
        name: "Hematology (EDTA K3) Tube",
        category: "Polymed",
        tagline: "EDTA K3 haematology tube — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/qk5eq62y_Hematology-EDTA-K3-Tube-85-600x819.webp",
        code: "SB-PM015",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "EDTA K3 haematology tube" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "K3 EDTA formulation for homogeneous mixing",
            "Whole blood haematology testing",
            "Immunohematology testing and blood donor screening",
        ],
        description:
            "Polymed Hematology EDTA K3 tubes use a K3 EDTA formulation for homogeneous mixing with the drawn sample. Suitable for whole-blood haematology, immunohematology testing and blood donor screening.",
    },
    {
        id: 22,
        slug: "hematology-edta-k2-tube",
        name: "Hematology (EDTA K2) Tube",
        category: "Polymed",
        tagline: "EDTA K2 haematology tube — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/44ikerje_Hematology-EDTA-K2-Tube-84-600x893.webp",
        code: "SB-PM016",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "EDTA K2 haematology tube" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "K2 EDTA formulation for homogeneous mixing",
            "Whole blood haematology testing",
            "Immunohematology testing and blood donor screening",
        ],
        description:
            "Polymed Hematology EDTA K2 tubes use a K2 EDTA formulation for homogeneous mixing with the drawn sample. Suitable for whole-blood haematology, immunohematology testing and blood donor screening.",
    },
    {
        id: 23,
        slug: "coagulation-citrate-tubes-double-wall",
        name: "Coagulation (Citrate) Tubes — Double Wall",
        category: "Polymed",
        tagline: "Double-wall coagulation tube — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/8yildxhm_Coagulation-Citrate-Tubes-Double-wall-83-600x766.webp",
        code: "SB-PM017",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "Double-wall coagulation tube" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "3.2% Buffered Sodium Citrate as anticoagulant",
            "Double-wall construction for added safety",
            "Suitable for examination of coagulation parameters",
        ],
        description:
            "Polymed Coagulation Citrate tubes (Double wall) contain 3.2% buffered sodium citrate solution as an anticoagulant. The double-wall build adds safety in transport and handling. Suitable for the examination of coagulation parameters such as PT and aPTT.",
    },
    {
        id: 24,
        slug: "coagulation-citrate-tubes-single-wall",
        name: "Coagulation (Citrate) Tubes — Single Wall",
        category: "Polymed",
        tagline: "Single-wall coagulation tube — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/5tznbxif_Coagulation-Citrate-Tubes-Single-wall-82-600x934.webp",
        code: "SB-PM018",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "Single-wall coagulation tube" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Sodium Citrate anticoagulant prevents clotting",
            "Preserves sample integrity",
            "Essential for PT (Prothrombin Time) and aPTT tests",
            "Assesses the coagulation pathway",
        ],
        description:
            "Polymed Coagulation Citrate tubes (Single wall) contain sodium citrate as an anticoagulant that prevents clotting and preserves sample integrity. Essential for PT (Prothrombin Time) and aPTT (Activated Partial Thromboplastin Time) tests that assess the coagulation pathway.",
    },
    {
        id: 25,
        slug: "serum-separating-tube-with-gel-separator",
        name: "Serum Separating Tube with Gel Separator",
        category: "Polymed",
        tagline: "Serum tube with gel — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/agq81hel_Serum-Separating-Tube-with-Gel-Separator-81-600x924.webp",
        code: "SB-PM019",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "Serum tube with gel" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Coated with micronised silica — acts as a clot activator",
            "Gel separates blood cells from serum in-tube",
            "Ideal for clinical serum tests and blood donor screening",
        ],
        description:
            "Polymed Serum Separating tubes are coated with micronised silica that acts as a clot activator. The in-tube gel cleanly separates blood cells from serum, making them ideal for clinical serum tests and blood donor screening chemistry work.",
    },
    {
        id: 26,
        slug: "serum-tubes",
        name: "Serum Tubes",
        category: "Polymed",
        tagline: "Plain serum tube — Polymed.",
        image: "https://customer-assets-39nsmqrw.emergentagent.net/job_lab-solutions-hub-1/artifacts/kedf9plh_Serum-Tubes-80-600x836.webp",
        code: "SB-PM020",
        specs: [
            { label: "Brand", value: "Polymed" },
            { label: "Type", value: "Plain serum tube" },
            { label: "Category", value: "Polymed" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Coated with micronised silica — acts as a clot activator",
            "Suitable for clinical serum tests",
            "Suitable for blood donor screening chemistry tests",
        ],
        description:
            "Polymed Serum tubes are coated with micronised silica that acts as a clot activator. Suitable for clinical serum tests and blood donor screening chemistry tests.",
    },
    {
        id: 27,
        slug: "ab-ctn-500gms",
        name: "AB Ctn 500gms",
        category: "Other",
        tagline: "Cotton — AB cotton.",
        image: "",
        code: "SB-OT021",
        specs: [
            { label: "Brand", value: "AB cotton" },
            { label: "Type", value: "Cotton" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "AB Ctn 500gms from AB cotton. Cotton — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 28,
        slug: "aspen-pregnency-card-100t",
        name: "ASPEN PREGNENCY CARD 100T",
        category: "Other",
        tagline: "Pregnancy cards — Aspen.",
        image: "",
        code: "SB-OT022",
        specs: [
            { label: "Brand", value: "Aspen" },
            { label: "Type", value: "Pregnancy cards" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "100T" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "ASPEN PREGNENCY CARD 100T from Aspen. Pregnancy cards — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 29,
        slug: "coverslip-22x22-mm-regular",
        name: "Coverslip-22x22 mm(Regular)",
        category: "Other",
        tagline: "Coverslip.",
        image: "",
        code: "SB-OT023",
        specs: [
            { label: "Type", value: "Coverslip" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Coverslip-22x22 mm(Regular). Coverslip — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 30,
        slug: "dbo-fbn-needle-22g",
        name: "DBO FBN Needle 22G",
        category: "Other",
        tagline: "Blood collection Needle — DBO.",
        image: "",
        code: "SB-OT024",
        specs: [
            { label: "Brand", value: "DBO" },
            { label: "Type", value: "Blood collection Needle" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "DBO FBN Needle 22G from DBO. Blood collection Needle — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 31,
        slug: "disposable-blood-collection-tubes",
        name: "DISPOSABLE BLOOD COLLECTION TUBES",
        category: "Other",
        tagline: "Ria Vial PP — Liv care.",
        image: "",
        code: "SB-OT025",
        specs: [
            { label: "Brand", value: "Liv care" },
            { label: "Type", value: "Ria Vial PP" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "DISPOSABLE BLOOD COLLECTION TUBES from Liv care. Ria Vial PP — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 32,
        slug: "disposable-cap-blue",
        name: "Disposable Cap ( Blue )",
        category: "Other",
        tagline: "Disposable Cap.",
        image: "",
        code: "SB-OT026",
        specs: [
            { label: "Type", value: "Disposable Cap" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Disposable Cap ( Blue ). Disposable Cap — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 33,
        slug: "disposable-esr-pipette",
        name: "DISPOSABLE ESR PIPETTE",
        category: "Other",
        tagline: "ESR Pipette — Liv care.",
        image: "",
        code: "SB-OT027",
        specs: [
            { label: "Brand", value: "Liv care" },
            { label: "Type", value: "ESR Pipette" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "DISPOSABLE ESR PIPETTE from Liv care. ESR Pipette — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 34,
        slug: "gloves-large",
        name: "GLOVES LARGE",
        category: "Other",
        tagline: "Gloves Latex — Unique.",
        image: "",
        code: "SB-OT028",
        specs: [
            { label: "Brand", value: "Unique" },
            { label: "Type", value: "Gloves Latex" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "GLOVES LARGE from Unique. Gloves Latex — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 35,
        slug: "gloves-medium",
        name: "GLOVES MEDIUM",
        category: "Other",
        tagline: "Gloves Latex — Unique.",
        image: "",
        code: "SB-OT029",
        specs: [
            { label: "Brand", value: "Unique" },
            { label: "Type", value: "Gloves Latex" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "GLOVES MEDIUM from Unique. Gloves Latex — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 36,
        slug: "hbsag-rapid-test-a-dx",
        name: "HBsAg Rapid Test A-Dx",
        category: "Other",
        tagline: "HbSag Rapid card — CTK.",
        image: "",
        code: "SB-OT030",
        specs: [
            { label: "Brand", value: "CTK" },
            { label: "Type", value: "HbSag Rapid card" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "HBsAg Rapid Test A-Dx from CTK. HbSag Rapid card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 37,
        slug: "hcv-rapid-test-a-dx",
        name: "HCV Rapid Test A-Dx",
        category: "Other",
        tagline: "HCV Rapid Card — CTK.",
        image: "",
        code: "SB-OT031",
        specs: [
            { label: "Brand", value: "CTK" },
            { label: "Type", value: "HCV Rapid Card" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "HCV Rapid Test A-Dx from CTK. HCV Rapid Card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 38,
        slug: "hiv-rapid-test-a-dx",
        name: "HIV Rapid Test A-Dx",
        category: "Other",
        tagline: "HIV Rapid Card — CTK.",
        image: "",
        code: "SB-OT032",
        specs: [
            { label: "Brand", value: "CTK" },
            { label: "Type", value: "HIV Rapid Card" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "HIV Rapid Test A-Dx from CTK. HIV Rapid Card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 39,
        slug: "isopopyl-rubing-alcohol",
        name: "ISOPOPYL RUBING ALCOHOL",
        category: "Other",
        tagline: "IPA Spirit 400ML — Klinik.",
        image: "",
        code: "SB-OT033",
        specs: [
            { label: "Brand", value: "Klinik" },
            { label: "Type", value: "IPA Spirit 400ML" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "ISOPOPYL RUBING ALCOHOL from Klinik. IPA Spirit 400ML — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 40,
        slug: "j-mitra-dengue-ns1-ag-25-test",
        name: "J.MITRA DENGUE NS1 AG 25 TEST",
        category: "Other",
        tagline: "Rapid card — J Mitra.",
        image: "",
        code: "SB-OT034",
        specs: [
            { label: "Brand", value: "J Mitra" },
            { label: "Type", value: "Rapid card" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "25 TEST" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "J.MITRA DENGUE NS1 AG 25 TEST from J Mitra. Rapid card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 41,
        slug: "jmitra-ns1-dengue-ag-ab-combi-card-1-card",
        name: "JMITRA  NS1 DENGUE AG& AB COMBI CARD (1 CARD)",
        category: "Other",
        tagline: "Rapid card — J Mitra.",
        image: "",
        code: "SB-OT035",
        specs: [
            { label: "Brand", value: "J Mitra" },
            { label: "Type", value: "Rapid card" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "JMITRA  NS1 DENGUE AG& AB COMBI CARD (1 CARD) from J Mitra. Rapid card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 42,
        slug: "mct-1-5ml-500nos-pkt",
        name: "MCT-1.5ML 500NOS/PKT",
        category: "Other",
        tagline: "1.5 ML MCT.",
        image: "",
        code: "SB-OT036",
        specs: [
            { label: "Type", value: "1.5 ML MCT" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "5ML" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "MCT-1.5ML 500NOS/PKT. 1.5 ML MCT — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 43,
        slug: "methanol-rankem-2-5ltr-m0151",
        name: "Methanol-Rankem-2.5ltr M0151",
        category: "Other",
        tagline: "Methanol — Rankem.",
        image: "",
        code: "SB-OT037",
        specs: [
            { label: "Brand", value: "Rankem" },
            { label: "Type", value: "Methanol" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "5ltr" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Methanol-Rankem-2.5ltr M0151 from Rankem. Methanol — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 44,
        slug: "mission-urine-strip-10p",
        name: "MISSION URINE STRIP 10P",
        category: "Other",
        tagline: "Urin strip 10P — Mission.",
        image: "",
        code: "SB-OT038",
        specs: [
            { label: "Brand", value: "Mission" },
            { label: "Type", value: "Urin strip 10P" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "MISSION URINE STRIP 10P from Mission. Urin strip 10P — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 45,
        slug: "mission-uristix-100-stris-2-para",
        name: "MISSION URISTIX 100 STRIS 2 PARA",
        category: "Other",
        tagline: "Urin strip 10P — Mission.",
        image: "",
        code: "SB-OT039",
        specs: [
            { label: "Brand", value: "Mission" },
            { label: "Type", value: "Urin strip 10P" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "100 STRIS" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "MISSION URISTIX 100 STRIS 2 PARA from Mission. Urin strip 10P — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 46,
        slug: "needle-holder-1-no",
        name: "NEEDLE HOLDER 1 NO",
        category: "Other",
        tagline: "Needle Holder.",
        image: "",
        code: "SB-OT040",
        specs: [
            { label: "Type", value: "Needle Holder" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "NEEDLE HOLDER 1 NO. Needle Holder — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 47,
        slug: "nitrile-examination-gloves-large-18",
        name: "Nitrile Examination Gloves Large 18%",
        category: "Other",
        tagline: "Nitrile Gloves.",
        image: "",
        code: "SB-OT041",
        specs: [
            { label: "Type", value: "Nitrile Gloves" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Nitrile Examination Gloves Large 18%. Nitrile Gloves — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 48,
        slug: "nitrile-examination-gloves-medium-18",
        name: "Nitrile Examination Gloves Medium 18%",
        category: "Other",
        tagline: "Nitrile Gloves.",
        image: "",
        code: "SB-OT042",
        specs: [
            { label: "Type", value: "Nitrile Gloves" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Nitrile Examination Gloves Medium 18%. Nitrile Gloves — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 49,
        slug: "nitrile-gloves-s-18",
        name: "Nitrile Gloves S 18%",
        category: "Other",
        tagline: "Nitrile Gloves.",
        image: "",
        code: "SB-OT043",
        specs: [
            { label: "Type", value: "Nitrile Gloves" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Nitrile Gloves S 18%. Nitrile Gloves — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 50,
        slug: "pasture-pipettes",
        name: "PASTURE PIPETTES",
        category: "Other",
        tagline: "3 ml Dropper.",
        image: "",
        code: "SB-OT044",
        specs: [
            { label: "Type", value: "3 ml Dropper" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "PASTURE PIPETTES. 3 ml Dropper — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 51,
        slug: "safeway-syringe-10-ml-w-o",
        name: "SAFEWAY SYRINGE 10 ML W/O",
        category: "Other",
        tagline: "Syringe — Safeway.",
        image: "",
        code: "SB-OT045",
        specs: [
            { label: "Brand", value: "Safeway" },
            { label: "Type", value: "Syringe" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "10 ML" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "SAFEWAY SYRINGE 10 ML W/O from Safeway. Syringe — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 52,
        slug: "safeway-syringe-2-ml-w-o",
        name: "SAFEWAY SYRINGE 2 ML W/O",
        category: "Other",
        tagline: "Syringe — Safeway.",
        image: "",
        code: "SB-OT046",
        specs: [
            { label: "Brand", value: "Safeway" },
            { label: "Type", value: "Syringe" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "2 ML" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "SAFEWAY SYRINGE 2 ML W/O from Safeway. Syringe — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 53,
        slug: "safeway-syringe-5-ml-w-o",
        name: "SAFEWAY SYRINGE 5 ML W/O",
        category: "Other",
        tagline: "Syringe — Safeway.",
        image: "",
        code: "SB-OT047",
        specs: [
            { label: "Brand", value: "Safeway" },
            { label: "Type", value: "Syringe" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "5 ML" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "SAFEWAY SYRINGE 5 ML W/O from Safeway. Syringe — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 54,
        slug: "sample-container-30ml-non-sterile",
        name: "Sample Container 30ml Non Sterile",
        category: "Other",
        tagline: "Urine container.",
        image: "",
        code: "SB-OT048",
        specs: [
            { label: "Type", value: "Urine container" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "30ml" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Sample Container 30ml Non Sterile. Urine container — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 55,
        slug: "sco-slides-1-35mm-50nos-pkt",
        name: "SCO Slides 1.35mm 50nos./Pkt.",
        category: "Other",
        tagline: "Glass Slides — SCO.",
        image: "",
        code: "SB-OT049",
        specs: [
            { label: "Brand", value: "SCO" },
            { label: "Type", value: "Glass Slides" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "50nos" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "SCO Slides 1.35mm 50nos./Pkt. from SCO. Glass Slides — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 56,
        slug: "sd-urocolor-10-para",
        name: "SD-UROCOLOR ( 10 PARA )",
        category: "Other",
        tagline: "Urine strip 10P — Abott.",
        image: "",
        code: "SB-OT050",
        specs: [
            { label: "Brand", value: "Abott" },
            { label: "Type", value: "Urine strip 10P" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "10 PARA" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "SD-UROCOLOR ( 10 PARA ) from Abott. Urine strip 10P — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 57,
        slug: "sodium-hypocloride-5-ltr",
        name: "SODIUM HYPOCLORIDE 5 LTR",
        category: "Other",
        tagline: "Sodium Hypo 4-6%.",
        image: "",
        code: "SB-OT051",
        specs: [
            { label: "Type", value: "Sodium Hypo 4-6%" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "5 LTR" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "SODIUM HYPOCLORIDE 5 LTR. Sodium Hypo 4-6% — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 58,
        slug: "soft-tissue-roll",
        name: "Soft Tissue Roll",
        category: "Other",
        tagline: "Tissue roll.",
        image: "",
        code: "SB-OT052",
        specs: [
            { label: "Type", value: "Tissue roll" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Soft Tissue Roll. Tissue roll — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 59,
        slug: "syphilis-strip-ab-rapidtrustline-100t",
        name: "SYPHILIS STRIP AB RAPIDTRUSTLINE 100T",
        category: "Other",
        tagline: "Rapid card — CTK.",
        image: "",
        code: "SB-OT053",
        specs: [
            { label: "Brand", value: "CTK" },
            { label: "Type", value: "Rapid card" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "100T" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "SYPHILIS STRIP AB RAPIDTRUSTLINE 100T from CTK. Rapid card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 60,
        slug: "tips-white-1000ul-500nos-pkt",
        name: "Tips-White-1000ul 500nos/Pkt.",
        category: "Other",
        tagline: "Tips.",
        image: "",
        code: "SB-OT054",
        specs: [
            { label: "Type", value: "Tips" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "500nos/Pkt" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Tips-White-1000ul 500nos/Pkt.. Tips — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 61,
        slug: "tips-white-200ul-1000nos-pkt",
        name: "Tips-White-200ul 1000nos/Pkt.",
        category: "Other",
        tagline: "Tips.",
        image: "",
        code: "SB-OT055",
        specs: [
            { label: "Type", value: "Tips" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "1000nos/Pkt" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "Tips-White-200ul 1000nos/Pkt.. Tips — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 62,
        slug: "tourniquet-belt",
        name: "TOURNIQUET BELT",
        category: "Other",
        tagline: "Tourniquet Belt.",
        image: "",
        code: "SB-OT056",
        specs: [
            { label: "Type", value: "Tourniquet Belt" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "TOURNIQUET BELT. Tourniquet Belt — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 63,
        slug: "troponin-i-rapid-a-dx-10-t",
        name: "TROPONIN I RAPID A-DX 10 T",
        category: "Other",
        tagline: "Rapid card — CTK.",
        image: "",
        code: "SB-OT057",
        specs: [
            { label: "Brand", value: "CTK" },
            { label: "Type", value: "Rapid card" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "10 T" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "TROPONIN I RAPID A-DX 10 T from CTK. Rapid card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 64,
        slug: "tydal-widal-slide-o-h-2-2-5-ml",
        name: "TYDAL WIDAL SLIDE(O,H)(2+2*5) ML",
        category: "Other",
        tagline: "Widal — Tulip.",
        image: "",
        code: "SB-OT058",
        specs: [
            { label: "Brand", value: "Tulip" },
            { label: "Type", value: "Widal" },
            { label: "Category", value: "Other" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "TYDAL WIDAL SLIDE(O,H)(2+2*5) ML from Tulip. Widal — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 65,
        slug: "typhoid-igg-igm-rapid-test-30t",
        name: "TYPHOID IGG/IGM RAPID TEST 30T",
        category: "Other",
        tagline: "Rapid card — CTK.",
        image: "",
        code: "SB-OT059",
        specs: [
            { label: "Brand", value: "CTK" },
            { label: "Type", value: "Rapid card" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "30T" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "TYPHOID IGG/IGM RAPID TEST 30T from CTK. Rapid card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 66,
        slug: "ultra-hbsag-card-sd-30t",
        name: "ULTRA HBsAg CARD SD 30T",
        category: "Other",
        tagline: "Rapid card — Ultra.",
        image: "",
        code: "SB-OT060",
        specs: [
            { label: "Brand", value: "Ultra" },
            { label: "Type", value: "Rapid card" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "30T" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "ULTRA HBsAg CARD SD 30T from Ultra. Rapid card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 67,
        slug: "ultra-hcv-test-card-30t-sd",
        name: "ULTRA HCV TEST CARD 30T SD",
        category: "Other",
        tagline: "Rapid card — Ultra.",
        image: "",
        code: "SB-OT061",
        specs: [
            { label: "Brand", value: "Ultra" },
            { label: "Type", value: "Rapid card" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "30T" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "ULTRA HCV TEST CARD 30T SD from Ultra. Rapid card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 68,
        slug: "ultra-hiv-1-2-card-30t-sd",
        name: "ULTRA HIV 1/2 CARD 30T SD",
        category: "Other",
        tagline: "Rapid card — Ultra.",
        image: "",
        code: "SB-OT062",
        specs: [
            { label: "Brand", value: "Ultra" },
            { label: "Type", value: "Rapid card" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "30T" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "ULTRA HIV 1/2 CARD 30T SD from Ultra. Rapid card — maintained in ready stock for same-week dispatch across India.",
    },
    {
        id: 69,
        slug: "urine-container-non-sterile-50ml-1-no",
        name: "URINE CONTAINER NON STERILE 50ML 1 NO",
        category: "Other",
        tagline: "Urine container.",
        image: "",
        code: "SB-OT063",
        specs: [
            { label: "Type", value: "Urine container" },
            { label: "Category", value: "Other" },
            { label: "Pack / Size", value: "50ML" },
            { label: "Availability", value: "Ready stock — enquire for pricing" },
        ],
        features: [
            "Quality-checked stock, ready for dispatch",
            "Consistent performance across batches",
            "Compatible with standard lab workflows",
        ],
        description:
            "URINE CONTAINER NON STERILE 50ML 1 NO. Urine container — maintained in ready stock for same-week dispatch across India.",
    },
];

export function getProductBySlug(slug) {
    return PRODUCTS.find((p) => p.slug === slug);
}

export function getRelated(product, count = 3) {
    return PRODUCTS.filter(
        (p) => p.category === product.category && p.id !== product.id
    ).slice(0, count);
}
